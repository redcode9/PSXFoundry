...
CLI="${1:-$REPOSITORY_ROOT/build/macos/dist/psxfoundry}"
CHDMAN="${PSXFOUNDRY_CHDMAN:-${POPFE_CHDMAN:-$REPOSITORY_ROOT/build/macos/helpers/chdman}}"
...
cleanup() {
    ...
    if [[ "${PSXFOUNDRY_KEEP_INTEGRATION:-${POPFE_KEEP_INTEGRATION:-0}}" == '1' ]]; then
...
(
    cd "$READ_ONLY_CWD"
    PSXFOUNDRY_HOME="$TEST_HOME" "$CLI" \
        "${COMMON_ARGS[@]}" \
        --psp-dir "$WORK_ROOT/psp" \
        --ps2-dir "$WORK_ROOT/ps2" \
        --ps3-pkg "$WORK_ROOT/ps3/PSXFoundry-Smoke.pkg" \
...
if find "$TEST_HOME/Library/Caches/PSXFoundry" \
